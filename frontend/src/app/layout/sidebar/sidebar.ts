...
@Component({
  ...
  template: `
    <aside
      class="flex shrink-0 flex-col border-r border-border bg-surface transition-[width] duration-200"
      [class.w-64]="!collapsed()"
      [class.w-16]="collapsed()"
    >
      <div
        class="flex h-14 items-center border-b border-border"
      ...
      <nav class="flex flex-1 flex-col gap-1 overflow-y-auto p-3" aria-label="Main navigation">
        @for (item of items(); track item.route) {
          <a
            ...
            class="flex items-center rounded-md px-3 py-2 text-sm font-medium text-muted transition-colors hover:bg-bg hover:text-fg"
      ...
      <div class="border-t border-border p-3">
        @if (collapsed()) {
          <div class="flex flex-col items-center gap-2">
            <a routerLink="/app/profile" [attr.title]="auth.currentUser()?.fullName">
              <img [src]="avatarSrc()" alt="Profile" width="36" height="36" class="h-9 w-9 rounded-full" />
            </a>
            <button
              type="button"
          ...
          <a
            ...
            class="flex items-center gap-3 rounded-md p-2 hover:bg-bg"
            routerLinkActive="bg-brand-50"
          >
          ...
          <button
            type="button"
            class="mt-1 flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs text-muted hover:bg-bg hover:text-fg"
            (click)="logout()"
          >
...
export class Sidebar {
  ...
  protected readonly items = computed<NavItem[]>(() =>
    visibleNav(this.auth, this.context.activeInstitution() !== null),
  );
